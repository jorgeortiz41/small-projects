...
public class MenuState extends State {
	...
	public void tick() {
		if(!creatingMap) {
			...
			if (mode.equals("Select")) {
				...
				//New Map
				uiManager.addObjects(new UIStringButton(handler.getWidth() / 2 - 64, (handler.getHeight() / 2) + (handler.getHeight() / 10) - (110), 128, 64, "New Map", () -> {
					if(!handler.isInMap()) {
						mode = "Menu";
				...
				//testMap1
				uiManager.addObjects(new UIStringButton(handler.getWidth() / 2 - 64, handler.getHeight() / 2 + (handler.getHeight() / 10) - 45, 128, 64, "Map 1", () -> {
					if(!handler.isInMap()) {
						mode = "Menu";
				...
				//testmap2
				uiManager.addObjects(new UIStringButton(handler.getWidth() / 2 - 64, (handler.getHeight() / 2) + (handler.getHeight() / 10) + (20), 128, 64, "Map 2", () -> {
					if(!handler.isInMap()) {
						mode = "Menu";
				...
				//other
				uiManager.addObjects(new UIStringButton(handler.getWidth() / 2 - 64, (handler.getHeight() / 2) + (handler.getHeight() / 10) + (86), 128, 64, "<Hello World>", () -> {
					if(!handler.isInMap()){
						mode = "Menu";
				...
				uiManager.addObjects(new UIStringButton(handler.getWidth() / 2 - 64, (handler.getHeight() / 2) + (handler.getHeight() / 10) + (150), 128, 64, "Multiplayer", () -> {
					if(!handler.isInMap()) {
						mode = "Menu";
						handler.setMap(MapBuilder.createMap(Images.testMap, handler));
						State.setState(handler.getGame().gameState);
	...
	private void tickNewScreen(){
		...
		if(handler.getKeyManager().keyJustPressed(KeyEvent.VK_8)){
			Cursor c = Toolkit.getDefaultToolkit().createCustomCursor(Images.tint(Images.Cursor,204,204,0), new Point(0, 0), "cursor1");
			display.getCanvas().setCursor(c);
			colorSelected = MapBuilder.star;
		}
		if(handler.getKeyManager().keyJustPressed(KeyEvent.VK_9)){
			Cursor c = Toolkit.getDefaultToolkit().createCustomCursor(Images.tint(Images.Cursor,169,169,169), new Point(0, 0), "cursor1");
			display.getCanvas().setCursor(c);
			colorSelected = MapBuilder.noteBlock;
		}
		if(handler.getKeyManager().keyJustPressed(KeyEvent.VK_C)){
			Cursor c = Toolkit.getDefaultToolkit().createCustomCursor(Images.tint(Images.Cursor,220,220,220), new Point(0, 0), "cursor1");
			display.getCanvas().setCursor(c);
